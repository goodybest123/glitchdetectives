...
import {
  ...
  Send, CheckCircle2, Wrench, Sparkles, RefreshCcw, Zap, Scissors, Disc3, BatteryCharging, Loader2,
  Radio, ScanSearch,
} from "lucide-react";
import { LEVEL2_GLITCHES, type Level2Glitch } from "@/lib/glitches-level2";
...
type View = "intro" | "mission-select" | "mission-1-repair";
type Phase =
  | "briefing"
  | "investigate"
  | "explainWrong"
...
const INTRO_TEXT =
  "Detective, it's chaos down here at the Half Repair Station! The factory's sharing machines are completely out of whack. They're supposed to split supplies fairly between the worker bots, but there's a logic glitch in their programming. The machines think they're making halves just because they're cutting things into two pieces. One robot gets a massive chunk of an energy bar, and the next robot gets a tiny crumb! Help us calibrate the laser cutters. Investigate the cuts, explain why they're unfair, and teach the machines what a true half really looks like.";

const MISSIONS = [
  { id: 1, name: "Half Repair Station", focus: "Repair unfair splits", unlocked: true, Icon: Scissors },
  { id: 2, name: "Quarter Calibration Bay", focus: "Master fourths", unlocked: false, Icon: Disc3 },
  { id: 3, name: "Power Cell Equalizer", focus: "Equal thirds & fifths", unlocked: false, Icon: BatteryCharging },
  { id: 4, name: "Final Share Audit", focus: "Mixed challenges", unlocked: false, Icon: Sparkles },
];

export default function FractionFactoryLevel2({ onExitToHub }: { onExitToHub: () => void }) {
        ...
        {view === "mission-select" && (
          <motion.div key="select" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            <MissionSelectView
              ...
              onStartMission1={() => setView("mission-1-repair")}
            />
          </motion.div>
        )}
        {view === "mission-1-repair" && (
    ...
    <div className="fixed bottom-5 right-5 z-40 flex items-center gap-2">
      {on && (
        ...
        >
          ...
          Listening: say "Start Scanner", "Next Item", "Back to Hub"
        </span>
      )}
      <button
        onClick={onToggle}
...
function IntroView({ voiceOn, onBack, onContinue }: { voiceOn: boolean; onBack: () => void; onContinue: () => void }) {
  useAutoSpeak(`Level 2: Half Repair Station. ${INTRO_TEXT}`);
  useVoiceCommands(
    {
  ...
  return (
    <>
      <TopBar title="Level 2: Half Repair Station" onBack={onBack} />
  ...
  return (
    <>
      ...
      <section className="max-w-5xl mx-auto px-4 sm:px-6 py-12">
        <p className="label-eyebrow" style={{ color: BLUE }}>Level 2 / Mission Select</p>
        ...
        <p className="text-sm mt-2 max-w-xl" style={{ color: "color-mix(in oklab, var(--color-brand-blue) 70%, white)" }}>
          Start with the Half Repair Station to fix the sharing machines.
        </p>
...
function Mission1View({ voiceOn, onBack, onFinish, onExitToHub }: { voiceOn: boolean; onBack: () => void; onFinish: () => void; onExitToHub: () => void }) {
  ...
  const shape: Level2Glitch = LEVEL2_GLITCHES[shapeIdx];
  const [vals, setVals] = useState<number[]>(shape.initialVals);
  const [repaired, setRepaired] = useState(false);
  const [repairMsg, setRepairMsg] = useState<string | null>(null);

  const isLast = shapeIdx === LEVEL2_GLITCHES.length - 1;

  const goToShape = (idx: number) => {
    const s = LEVEL2_GLITCHES[idx];
    setShapeIdx(idx);
  ...
  const robotLine = useMemo(() => {
    switch (phase) {
      ...
      case "repairPrompt": return "You spotted the glitch! I can't recalibrate alone — will you help me fix the laser?";
      case "repair": return shape.robotRepair;
      case "teach": return shape.robotExplain;
      case "shapeDone": return shape.robotSuccess;
      case "missionDone": return "Mission complete! Every sharing machine is fair again — you taught them what a true half looks like!";
      default: return "";
    }
  }, [phase, shape]);
  ...
  return (
    <>
      <TopBar title={`Mission 1 — Item ${shapeIdx + 1} of ${LEVEL2_GLITCHES.length}`} onBack={onBack} backLabel="Missions" />
      <section className="max-w-6xl mx-auto px-4 sm:px-6 py-8 grid lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl border p-6 shadow-sm flex flex-col" style={{ borderColor: "color-mix(in oklab, var(--color-brand-blue) 12%, white)" }}>
          ...
          <div className="flex-1 flex items-center justify-center min-h-[280px] mt-4">
            ...
            ) : phase === "repair" && shape.mechanic === "snap" ? (
              <motion.div
                key={`${shape.id}-snap`}
                initial={{ scale: 0.96, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
          ...
          {phase === "repair" && shape.mechanic === "range" && (
    ...
    return (
      <div className="space-y-3">
        <p className="text-xs font-mono text-center" style={{ color: "color-mix(in oklab, var(--color-brand-blue) 70%, white)" }}>
          Tap Start Scanner so we can inspect the cut together.
        </p>
        <motion.button
    ...
    return (
      <div className="flex flex-col items-center gap-4 text-center">
        <p className="text-sm" style={{ color: "color-mix(in oklab, var(--color-brand-blue) 70%, white)" }}>
          Let's calibrate the laser cutter and split this fairly.
        </p>
        <motion.button
  ...
  if (phase === "shapeDone") {
    return (
      <motion.div initial={{ scale: 0.96 }} animate={{ scale: [0.96, 1.04, 1] }} className="flex flex-col items-center gap-4">
        ...
        <p className="font-semibold text-lg" style={{ color: BLUE }}>Item repaired!</p>
        <button
          onClick={props.onNextShape}
          className="inline-flex items-center gap-2 px-5 py-3 rounded-xl font-semibold transition-transform hover:scale-[1.02]"
          style={{ background: BLUE, color: "white" }}
        >
          {props.isLast ? "Finish Mission" : "Next Item"} <ArrowRight className="w-4 h-4" />
        </button>
      </motion.div>
    );
  }
  ...
  if (phase === "missionDone") {
    return (
      <div className="flex flex-col items-center gap-4 text-center">
        ...
        <p className="text-sm" style={{ color: "color-mix(in oklab, var(--color-brand-blue) 70%, white)" }}>
          Every sharing machine is calibrated. ZED-4 finally understands what a true half means.
        </p>
        <button
