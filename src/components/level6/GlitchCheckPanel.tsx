...
/** Level 6 Glitch Check — every case is a glitch. */
export function L6GlitchCheckPanel({
  caseDef,
  onZedSpeak,
  onResolved,
}: {
  ...
  const PROMPT =
    "Inspect ZED's reasoning in the case file. Is the Nexus translation correct, or is there a glitch?";
  ...
  const handleSayRight = () => {
    const line = "Are you sure, architect? Look at the quantities one more time…";
    setNudge(line);
    onZedSpeak?.(line);
    speakText(line);
  };
  ...
  const handleSayGlitch = () => {
    const line = "Glitch detected — let's restore the Nexus together!";
    onZedSpeak?.(line);
    ...
    <div className="flex flex-col gap-5">
      <header className="flex items-start justify-between gap-3 flex-wrap">
        <div className="flex-1 min-w-[200px]">
          <p className="label-eyebrow text-cyan-200/80">Glitch check · Phase 0</p>
          <h3 className="text-2xl font-bold text-cyan-50 mt-1">
            Correct translation — or Nexus glitch?
          </h3>
